...
class TestMainPageQuestions:
    ...
    @allure.title('Восстановление пароля: Проверка перехода на страницу восстановления пароля по кнопке «Восстановить пароль».')
    def test_restore_link_redirect_to_restore_form(self, driver):
        ...
        restore_password_page.click_random_place(driver)
    ...
    @allure.title('Восстановление пароля: Проверка клик по кнопке показать/скрыть пароль делает поле активным — подсвечивает его.')
